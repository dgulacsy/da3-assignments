...
```{r}
df <- read_rds("data/clean/bisnode_clean.rds")
...
formula_knn = formula(paste0(target_f, paste0(" ~ ",paste(c(business, financial_basic, flags),collapse = " + "))))  # ceo,sales, financial_ext, financial_basic_ratios, 
formula_enet = formula(paste0(target_f, paste0(" ~ ",paste(c(business, ceo,sales, financial_basic, financial_ext, financial_basic_ratios, flags),collapse = " + "))))
```
...
```{r}
set.seed(1234)
trctrl <- trainControl(method = "repeatedcv", number = 3, repeats = 1, verboseIter = TRUE)
