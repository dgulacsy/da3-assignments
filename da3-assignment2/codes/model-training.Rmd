...
Their CV performance can be seen **below**.

Next, we ran a cross validation on choosing the loss function with the minimal expected loss. The results can be seen **below**. 
...
data_test$rf_no_loss_pred <- predict(model_rf, newdata = data_test, type = "prob")$fast_growth
data_test$logit_no_loss_pred <- predict(model_logit, newdata = data_test, type = "prob")$fast_growth

calib_gbm<-create_calibration_plot(data_test, 
  file_name = "best-model-calibration", 
  prob_var = "gbm_no_loss_pred", 
  actual_var = "is_fg",
  n_bins = 10)

calib_rf<-create_calibration_plot(data_test, 
  file_name = "best-model-calibration", 
...
RMSE(data_test$gbm_p_prediction, data_test$is_fg)

# ROC curve on holdout
roc_obj_test <- roc(data_test$is_fg, data_test[, "gbm_p_prediction", drop=TRUE])

# AUC
as.numeric(roc_obj_test$auc)
